<template>
  ...
  <v-container>
    ...
    <v-card class="mx-auto my-5" max-width="355" outlined>
      <v-list-item>
        <v-list-item-content>
          <div class="overline mb-4">DASH ACCOUNT</div>
    ...
    <v-card
      v-if="!curActionRequest.actionRequired"
      class="mx-auto my-5"
      max-width="355"
      outlined
    ...
    >
      <v-list-item>
        <v-list-item-content>
          ...
          <div class="overline mb-4">DAPP PIN</div>
          <v-list-item-title class="headline mb-1 text-center">
    ...
    <v-card class="mx-auto my-5" max-width="355" outlined>
      <v-list-item>
        <v-list-item-content>
          <div class="overline mb-4">BALANCE</div>
          <v-list-item-title class="headline mb-1 text-center">
            {{ this.$store.state.balanceInDash }} Dash
          </v-list-item-title>
        </v-list-item-content>
      </v-list-item>
      <v-card-actions class="text-center">
        <v-btn style="width: 45%" @click="receiveDialog = true"
          ><v-icon left>mdi-download</v-icon> Receive</v-btn
        >
        <v-spacer />
        <v-btn nuxt to="/scanuser" style="width: 45%"
          >Scan <v-icon color="green" right>mdi-qrcode-scan</v-icon></v-btn
        >
      </v-card-actions>
    </v-card>
    <v-card class="mx-auto my-5" max-width="355" outlined>
      <v-list-item>
        <v-list-item-content>
          <div class="overline">Recent Payments</div>
        </v-list-item-content>
      </v-list-item>
    ...
    <v-dialog v-model="receiveDialog" persistent max-width="600px">
      <v-card>
        <v-card-title>
          <span class="headline">Receive</span>
        </v-card-title>
        <v-card-text>
          <v-container>
            <v-row>
              <v-col class="text-center">
                <qrcode :value="$store.state.fundingAddress"></qrcode>

                <v-list-item>
                  <v-list-item-content>
                    <v-list-item-subtitle>
                      <v-text-field
                        :value="$store.state.fundingAddress"
                        class="mt-1"
                        readonly=""
                        outlined
                      ></v-text-field>
...
import PaymentRequestsHistory from '@/components/paymentRequestsHistory.vue'
const sleep = (ms) => new Promise((resolve) => setTimeout(resolve, ms))

Vue.component(VueQrcode.name, VueQrcode)
export default {
  components: { PaymentRequestsHistory },
  data() {
    return {
      loginPinTimeLeft: 0,
  ...
  methods: {
    ...
    async pollRefreshWalletBalance() {
      if (this.$router.currentRoute.path != '/home') return
      this.$store.dispatch('refreshWalletBalance')
      await sleep(1000)
      this.pollRefreshWalletBalance()
    },
